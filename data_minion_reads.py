...
def recherche_fichiers(*origines, un_fichier=False):
...
def lire_taille():
	...
	return json.load(open(os.path.join(cache, 'tailles', 'SRR8691535.fastq.json')))


def lire_taille_aleatoire():
	recherche_fichiers(
		os.path.abspath(os.path.join(__file__, '..', 'Donnees')),
		un_fichier=False
	)
	while True:
		x = json.load(open(os.path.join(cache, 'tailles', random.choice(os.listdir(os.path.join(cache, 'tailles'))))))
		if len(x) > 20000:
			return x
